...
describe Blacklight::Solr::Doc::Ext::Marc::Citation do
  before(:all) do
    
    klass = Blacklight::Solr::Doc::Ext::Marc::Document
    
    @record_with_standard_citation_data = klass.new(standard_citation)
    
    @record_without_245b = klass.new(record1_xml)
    
    @record_without_authors = klass.new(record2_xml)
    
    @record_with_4plus_authors = klass.new(record3_xml)
    
    @record_without_citable_data = klass.new(no_good_data_xml)
  end
